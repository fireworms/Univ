...
public class DepartmentAction {
	...
	public void setListeners(DepartmentView view) {
		view.select.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					ArrayList<DepData> dep = view.db.select(view.searchCondition
							.getSelectedItem().toString(), view.searchBox.getText());
		...
		view.insert.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (nullCheck(view)) {
					try {
						view.db.insert(view.depCodeTf.getText(), view.depNameTf.getText(),
		...
		view.remove.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (view.tableSelected) {
					try {
						if (view.tableSelected) {
							view.db.remove(view.table.getValueAt(view.index, 0).toString());
							showTable(view);
					...
					if (indexString != null && indexString != "") {
						try {
							...
							String name = view.dtm.getValueAt(view.index, 0).toString();
							view.db.remove(name);
			...
			public void actionPerformed(ActionEvent e) {
				if (view.isModifying) {
						...
						if (JOptionPane.showConfirmDialog(null, "수정하실겁니까요") == 0) {
							try {
								view.db.modify(view.depNameTf.getText(),
										view.depMajorTf.getText(),
										view.table.getValueAt(view.index, 0).toString());
								showTable(view);
								clearField(view);
					...
					if (view.tableSelected) {
						...
						view.depCodeTf.setText(view.table.getValueAt(view.index, 0).toString());
		...
		view.table.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent arg0) {
				...
				view.depCodeTf.setText(view.table.getValueAt(view.index, 0).toString());
				view.depNameTf.setText(view.table.getValueAt(view.index, 1).toString());
				view.depMajorTf.setText(view.table.getValueAt(view.index, 2).toString());
			}
		});
	...
	public boolean nullCheck(DepartmentView view) {
	...
	private void showTable(DepartmentView view) throws Exception{
	...
	public void clearField(DepartmentView view) {
